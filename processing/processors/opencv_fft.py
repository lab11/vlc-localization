...
def dbg_plot_subplots(fname):
	with logger.scoped_op('plot_subplots for ' + fname):
...
def imag_proc(file_name, num_of_tx, camera, debug):
	...
	with logger.scoped_op("Loading image"):
	...
	# Handle orientation
	with logger.scoped_op("Normalizing image rotation"):
	...
	# Blur image
	with logger.scoped_op("Applying blur"):
	...
	with logger.scoped_op("Threshold image"):
	...
	with logger.scoped_op("Locate transmitters"):
	...
	with logger.scoped_op("Computing transmitter frequencies"):
